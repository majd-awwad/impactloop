...
const TEST_MARKER = '[test-admin-incident-reports-export-http]';
...
describe('admin incident reports export HTTP auth', () => {
  test('preflight returns 401 without token', async () => {
    const result = await request('/api/admin/no-show-reports/export/preflight');
  ...
  test('preflight succeeds for admin without pdf format', async () => {
    const result = await request(
      `/api/admin/no-show-reports/export/preflight?search=${encodeURIComponent(TEST_MARKER)}`,
      { token: adminToken },
    );
  ...
  test('export csv succeeds for admin', async () => {
    const result = await request(
      `/api/admin/no-show-reports/export?format=csv&search=${encodeURIComponent(`${TEST_MARKER}-none`)}`,
      { token: adminToken },
    );
    assert.equal(result.status, 200);
  ...
  test('export xlsx succeeds for admin', async () => {
    const result = await request(
      `/api/admin/no-show-reports/export?format=xlsx&search=${encodeURIComponent(`${TEST_MARKER}-none`)}`,
      { token: adminToken },
    );
    assert.equal(result.status, 200);
    assert.match(
  ...
  test('export pdf is rejected for incident reports', async () => {
    const result = await request(
      '/api/admin/no-show-reports/export?format=pdf',
      { token: adminToken },
    );
