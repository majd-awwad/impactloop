...
const TEST_MARKER = '[test-admin-material-reports-export-http]';
...
describe('admin material reports export HTTP auth', () => {
  ...
  test('preflight succeeds for admin without pdf format', async () => {
    const result = await request(
      '/api/admin/material-reports/export/preflight?status=PENDING',
      { token: adminToken },
    );
  ...
  test('export csv succeeds for admin', async () => {
    const result = await request(
      '/api/admin/material-reports/export?format=csv&status=PENDING&search=no-such-marker-xyz',
      { token: adminToken },
    );
    assert.equal(result.status, 200);
  ...
  test('export xlsx succeeds for admin', async () => {
    const result = await request(
      '/api/admin/material-reports/export?format=xlsx&status=PENDING&search=no-such-marker-xyz',
      { token: adminToken },
    );
    assert.equal(result.status, 200);
    assert.match(
  ...
  test('export pdf is rejected for material reports', async () => {
    const result = await request(
      '/api/admin/material-reports/export?format=pdf&status=PENDING',
      { token: adminToken },
    );
