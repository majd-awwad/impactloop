...
const TEST_MARKER = '[test-admin-materials-export-http]';
...
describe('admin materials export HTTP auth', () => {
  test('preflight returns 401 without token', async () => {
    const result = await request('/api/admin/materials/export/preflight');
  ...
  test('export csv succeeds for admin', async () => {
    const result = await request(
      '/api/admin/materials/export?format=csv&search=no-such-marker-xyz',
      { token: adminToken },
    );
    assert.equal(result.status, 200);
  ...
  test('export xlsx succeeds for admin', async () => {
    const result = await request(
      '/api/admin/materials/export?format=xlsx&search=no-such-marker-xyz',
      { token: adminToken },
    );
    assert.equal(result.status, 200);
    assert.match(
  ...
  test('export pdf is rejected for materials', async () => {
    const result = await request(
      '/api/admin/materials/export?format=pdf&search=no-such-marker-xyz',
      { token: adminToken },
    );
