...
const TEST_MARKER = '[test-admin-people-export-http]';
...
describe('admin people export HTTP auth', () => {
  test('preflight returns 401 without token', async () => {
    const result = await request('/api/admin/people/export/preflight');
  ...
  test('preflight succeeds for admin without pdf format', async () => {
    const result = await request(
      `/api/admin/people/export/preflight?search=${encodeURIComponent(TEST_MARKER)}`,
      { token: adminToken },
    );
  ...
  test('export csv succeeds for admin', async () => {
    const result = await request(
      `/api/admin/people/export?format=csv&search=${encodeURIComponent(`${TEST_MARKER}-none`)}`,
      { token: adminToken },
    );
    assert.equal(result.status, 200);
  ...
  test('export xlsx succeeds for admin', async () => {
    const result = await request(
      `/api/admin/people/export?format=xlsx&search=${encodeURIComponent(`${TEST_MARKER}-none`)}`,
      { token: adminToken },
    );
    assert.equal(result.status, 200);
    assert.match(
  ...
  test('export pdf is rejected for people', async () => {
    const result = await request('/api/admin/people/export?format=pdf', {
      token: adminToken,
    });
