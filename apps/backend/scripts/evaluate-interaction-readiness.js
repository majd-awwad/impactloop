...
export const classifyOrigin = (input) => {
    ...
    if (eventSource === "LOAD_TEST")
        return "BENCHMARK";
    if (eventSource === "TEST")
        return "TEST_FIXTURE";
    if (eventSource === "DEMO_SEED")
        return "DEMO_SEED";
    if (eventSource === "SYNTHETIC")
        return "SYNTHETIC";
    if (eventSource === "LEGACY_UNCLASSIFIED")
        return "UNKNOWN";
    if (input.recommendationObservability) {
        if (eventSource === "REAL" && accountEligible)
            return "REAL_USER";
        return "UNKNOWN";
    }
    // Product-domain interactions without eventSource: account eligibility only.
    if (eligibility === "EXCLUDED_DEMO")
        return "DEMO_SEED";
    if (eligibility === "EXCLUDED_TEST")
        return "TEST_FIXTURE";
    if (eligibility === "EXCLUDED_INTERNAL")
        return "UNKNOWN";
    if (accountEligible)
        return "REAL_USER";
    return "UNKNOWN";
...
const reversal = (signal) => ["UNLIKE", "UNSAVE", "UNFOLLOW"].includes(signal);
...
export const resolveInteractionRecords = (raw) => {
    const ordered = [...raw].sort((a, b) => a.occurredAt.getTime() - b.occurredAt.getTime() ||
    ...
    for (const row of ordered) {
        ...
        if (["LIKE", "UNLIKE", "SAVE", "UNSAVE", "FOLLOW", "UNFOLLOW"].includes(row.signal)) {
            const family = row.signal === "LIKE" || row.signal === "UNLIKE"
                ? "LIKE"
        ...
        const operationKey = row.operationId
            ...
            : null;
        if (operationKey && operationSeen.has(operationKey))
            continue;
        if (operationKey)
            operationSeen.add(operationKey);
    ...
    for (const [key, rows] of stateEvents) {
        const final = [...rows].sort((a, b) => b.occurredAt.getTime() - a.occurredAt.getTime() ||
    ...
    return output.sort((a, b) => a.occurredAt.getTime() - b.occurredAt.getTime() ||
        a.dedupeKey.localeCompare(b.dedupeKey));
};
export const hasOperationLeakage = (train, test) => {
...
export const buildMatrixMetrics = (users, items, pairs) => {
    const userItems = new Map(users.map((user) => [user, new Set()]));
    const itemUsers = new Map(items.map((item) => [item, new Set()]));
    const unique = new Set();
    for (const pair of pairs) {
        unique.add(`${pair.userId}|${pair.itemId}`);
        if (!userItems.has(pair.userId))
            userItems.set(pair.userId, new Set());
        if (!itemUsers.has(pair.itemId))
            itemUsers.set(pair.itemId, new Set());
        userItems.get(pair.userId).add(pair.itemId);
        itemUsers.get(pair.itemId).add(pair.userId);
    }
    const below = (values, threshold) => [...values].filter((set) => set.size < threshold).length;
...
export const countItemsWithAtLeastUsers = (matrix, threshold) => {
    const totalItems = matrix.items;
    if (threshold === 2)
        return totalItems - matrix.itemsBelow["2"];
    if (threshold === 3)
        return totalItems - matrix.itemsBelow["3"];
    return totalItems - matrix.itemsBelow["5"];
};
export const buildGraphMetrics = (pairs) => {
    ...
    for (const pair of pairs) {
        ...
        userItems.get(pair.userId).add(pair.itemId);
        itemUsers.get(pair.itemId).add(pair.userId);
        const user = `u:${pair.userId}`, item = `i:${pair.itemId}`;
        if (!adjacency.has(user))
            adjacency.set(user, new Set());
        if (!adjacency.has(item))
            adjacency.set(item, new Set());
        ...
        for (let i = 0; i < keys.length; i++)
            for (let j = i + 1; j < keys.length; j++) {
                ...
                for (const value of sets.get(keys[i]))
                    if (sets.get(keys[j]).has(value))
                        count++;
                if (count)
                    result.push({ left: keys[i], right: keys[j], count });
            }
        return result
    ...
    return {
        ...
        isolatedUsers: 0,
        isolatedItems: 0,
        userDegree: summarizeDistribution([...userItems.values()].map((set) => set.size)),
        itemDegree: summarizeDistribution([...itemUsers.values()].map((set) => set.size)),
        itemPairCooccurrence: overlaps(itemUsers),
        userPairOverlap: overlaps(userItems),
    };
};
const pseudonymizeGraph = (graph) => {
    const pseudonym = (value) => createHash("sha256").update(value).digest("hex").slice(0, 16);
    return {
        ...graph,
...
const bounded = async (rows, total) => ({
    rows: await rows,
    total,
    capped: total > MAX_PROFILE_ROWS,
});
const statusSignal = (status) => status === "COMPLETED"
            ...
            : status === "REJECTED"
                ...
                : status === "EXPIRED"
                    ...
                    : ["NO_SHOW", "FULFILLMENT_FAILED"].includes(status)
                        ? "RESERVATION_FAILED"
                        : "RESERVATION_CREATED";
export async function profileDatabase(client = prisma) {
    ...
    const [views, likes, reservations, histories, projectLikes, saves, follows, builds, impressions, actions,] = await Promise.all([
        bounded(client.materialView.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                materialId: true,
        ...
        bounded(client.materialLike.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                materialId: true,
        ...
        bounded(client.reservation.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                materialId: true,
        ...
        bounded(client.reservationStatusHistory.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                reservationId: true,
        ...
        bounded(client.projectLike.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                projectId: true,
        ...
        bounded(client.projectSave.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                projectId: true,
        ...
        bounded(client.projectFollow.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                projectId: true,
        ...
        bounded(client.projectBuild.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                projectId: true,
        ...
        bounded(client.recommendationImpression.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                learnerId: true,
        ...
        bounded(client.recommendationAction.findMany({
            take: MAX_PROFILE_ROWS,
            select: {
                id: true,
                learnerId: true,
    ...
    const originForUser = (userId, extras) => classifyOrigin({
    ...
    const allObserved = resolved.filter((row) => row.userId && row.itemId && row.entityType);
    const real = resolved.filter((row) => row.eligible &&
        row.origin === "REAL_USER" &&
        row.userId &&
        row.itemId &&
        row.entityType);
    const pairs = (entity) => real
    ...
    const allPairs = (entity) => allObserved
        .filter((row) => row.entityType === entity)
        .map((row) => ({
        userId: row.userId,
        itemId: row.itemId,
    ...
    const grouped = (values, key) => [...new Set(values.map(key))].map((value) => values.filter((row) => key(row) === value));
    const dist = (groups, unique = false) => summarizeDistribution(groups.map((group) => unique
        ? new Set(group.map((row) => `${row.entityType}:${row.itemId}`)).size
        : group.length));
    const usersAtLeast = (groups, threshold) => groups.filter((group) => new Set(group.map((row) => `${row.entityType}:${row.itemId}`)).size >=
        threshold).length;
    const userGroups = grouped(real, (row) => row.userId);
    const materialGroups = materialIds.map((id) => real.filter((row) => row.entityType === "MATERIAL" && row.itemId === id));
    const projectGroups = projectIds.map((id) => real.filter((row) => row.entityType === "PROJECT" && row.itemId === id));
    ...
    for (const row of real) {
        if (!daysByUser.has(row.userId))
            daysByUser.set(row.userId, new Set());
        daysByUser.get(row.userId).add(utcDay(row.occurredAt));
        const key = `${row.entityType}:${row.itemId}`;
        if (!daysByItem.has(key))
            daysByItem.set(key, new Set());
    ...
    const beforePublication = resolved.filter((row) => {
        const item = row.entityType === "MATERIAL"
    ...
    const actionBeforeImpression = actions.rows.filter((row) => row.actionAt < row.impression.shownAt).length;
    const attribution = {};
    for (const row of actions.rows) {
    ...
    const queueStatusCounts = Object.fromEntries(["PENDING", "PROCESSING", "RETRY", "PROCESSED", "DEAD"].map((status) => [
        status,
    ...
    const materialRealUsers = new Set(pairs("MATERIAL").map((pair) => pair.userId));
    const projectRealUsers = new Set(pairs("PROJECT").map((pair) => pair.userId));
    const allUsers = [...new Set(allObserved.map((row) => row.userId))];
    const materialAllUsers = new Set(allPairs("MATERIAL").map((pair) => pair.userId));
    const projectAllUsers = new Set(allPairs("PROJECT").map((pair) => pair.userId));
    ...
    const profile = {
        ...
        populations: {
            ...
            availableMaterials: materials.filter((item) => item.status === "AVAILABLE").length,
            ...
            usersWithBothDomains: [...materialAllUsers].filter((id) => projectAllUsers.has(id)).length,
            usersWithBothRealDomains: [...materialRealUsers].filter((id) => projectRealUsers.has(id)).length,
            coldUsers: users.filter((user) => !allUsers.includes(user.id)).length,
            coldRealUsers: users.filter((user) => !activeUsers.includes(user.id))
                .length,
            coldMaterials: materialIds.filter((id) => !allPairs("MATERIAL").some((pair) => pair.itemId === id)).length,
            coldRealMaterials: materialIds.filter((id) => !pairs("MATERIAL").some((pair) => pair.itemId === id)).length,
            coldProjects: projectIds.filter((id) => !allPairs("PROJECT").some((pair) => pair.itemId === id)).length,
            coldRealProjects: projectIds.filter((id) => !pairs("PROJECT").some((pair) => pair.itemId === id)).length,
        },
        origins: {
        ...
        distributions: {
            ...
            activityDaysPerUser: summarizeDistribution([...daysByUser.values()].map((set) => set.size)),
            activeTimeSpanDays: first && last
                ? Math.ceil((last.getTime() - first.getTime()) / 86_400_000)
                : 0,
            repeatInteractionRate: real.length
                ? 1 -
                    new Set(real.map((row) => `${row.userId}|${row.entityType}|${row.itemId}`)).size /
                        real.length
                : 0,
        },
        allCurrentRows: {
            interactionsPerUser: dist(grouped(allObserved, (row) => row.userId)),
            uniqueItemsPerUser: dist(grouped(allObserved, (row) => row.userId), true),
            interactionsPerMaterial: dist(materials.map((item) => allObserved.filter((row) => row.entityType === "MATERIAL" && row.itemId === item.id))),
            interactionsPerProject: dist(projects.map((item) => allObserved.filter((row) => row.entityType === "PROJECT" && row.itemId === item.id))),
        },
        matrices: {
            ...
            combined: buildMatrixMetrics(activeUsers, [
            ...
            allCurrentRows: {
                material: buildMatrixMetrics(allUsers, materials.map((item) => item.id), allPairs("MATERIAL")),
                project: buildMatrixMetrics(allUsers, projects.map((item) => item.id), allPairs("PROJECT")),
            },
        },
        graph: {
        ...
        stateResolution: {
            ...
            viewsCapped: raw.filter((row) => row.signal === "VIEW").length -
                resolved.filter((row) => row.signal === "VIEW").length,
            reversals: resolved.filter((row) => row.reversal).length,
            reservationStates: Object.fromEntries([...new Set(reservations.rows.map((row) => row.status))]
                .sort()
                .map((status) => [
                status,
                reservations.rows.filter((row) => row.status === status).length,
            ])),
            buildStates: Object.fromEntries([...new Set(builds.rows.map((row) => row.status))]
                .sort()
                .map((status) => [
                status,
        ...
        attribution: {
            ...
            actionOrigins: actions.rows.reduce((acc, row) => {
        ...
        temporal: {
            ...
            usersActiveMultipleDays: [...daysByUser.values()].filter((set) => set.size > 1).length,
            itemsActiveMultipleDays: [...daysByItem.values()].filter((set) => set.size > 1).length,
            interactionsBeforeItemPublication: beforePublication,
            impressionBeforeItemAvailability,
            actionBeforeImpression,
            futureTimestamps: resolved.filter((row) => row.occurredAt.getTime() > Date.now() + 300_000).length,
            duplicateTimestampRows: raw.length -
                new Set(raw
                    .filter((row) => row.userId && row.itemId)
                    .map((row) => `${row.userId}|${row.entityType}|${row.itemId}|${row.signal}|${row.occurredAt.toISOString()}`)).size,
            impossibleReservationOrdering: reservations.rows.filter((row) => row.updatedAt < row.createdAt ||
                Boolean(row.completedAt &&
                    row.acceptedAt &&
                    row.completedAt < row.acceptedAt) ||
                Boolean(row.cancelledAt && row.cancelledAt < row.createdAt)).length,
            projectBuildOrdering: builds.rows.filter((row) => row.updatedAt < row.createdAt ||
                Boolean(row.completedAt && row.completedAt < row.startedAt)).length,
        },
        leakage: {
            seededRows: resolved.filter((row) => row.origin === "DEMO_SEED").length,
            recommendationGeneratedRows: resolved.filter((row) => row.source.startsWith("recommendation_")).length,
            workflowCopyItems: materials.filter((item) => hasMarker(item.title, ["(spare batch)"])).length,
            linkedBuildItems: builds.rows.reduce((sum, build) => sum + build.items.filter((item) => item.status !== "MISSING").length, 0),
            durableOperationDeduplication: true,
        },
        splitEligibility: {
            usersWithAtLeast2MaterialItems: new Set(activeUsers.filter((userId) => new Set(pairs("MATERIAL")
                .filter((pair) => pair.userId === userId)
                .map((pair) => pair.itemId)).size >= 2)).size,
            usersWithAtLeast5MaterialItems: new Set(activeUsers.filter((userId) => new Set(pairs("MATERIAL")
                .filter((pair) => pair.userId === userId)
                .map((pair) => pair.itemId)).size >= 5)).size,
            usersWithAtLeast10MaterialItems: new Set(activeUsers.filter((userId) => new Set(pairs("MATERIAL")
                .filter((pair) => pair.userId === userId)
                .map((pair) => pair.itemId)).size >= 10)).size,
            leaveOneOutUsersMaterial: new Set(activeUsers.filter((userId) => new Set(pairs("MATERIAL")
                .filter((pair) => pair.userId === userId)
                .map((pair) => pair.itemId)).size >= 2)).size,
            temporalPerUserUsersMaterial: new Set(activeUsers.filter((userId) => new Set(pairs("MATERIAL")
                .filter((pair) => pair.userId === userId)
                .map((pair) => pair.itemId)).size >= 2)).size,
            realTrainableRows: real.length,
        },
...
export const formatProgress = (profile) => {
