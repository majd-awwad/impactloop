...
isolatedRecommendationTest('unknown mode is rejected with non-zero exit code', async () => {
...
isolatedRecommendationTest('artifact failure returns non-zero exit code', async () => {
...
isolatedRecommendationTest('flags and artifact paths restore after success', async () => {
  if (!hasDatabase) return;
  ...
  try {
    ...
    env.recommendationMlProjectArtifactPath = '/tmp/preflight-test-artifact.json';
...
isolatedRecommendationTest('flags and artifact paths restore after failure', async () => {
  const original = captureRecommendationFlags(env);
  try {
    env.recommendationMlShadowEnabled = true;
    const priorBeforeRun = captureRecommendationFlags(env);
...
isolatedRecommendationTest('process.env keys restore after success with absent and empty semantics', async () => {
  ...
  try {
    ...
    process.env.RECOMMENDATION_ML_MATERIAL_ARTIFACT_PATH = '/tmp/material-artifact.json';
    ...
    assert.equal('PREFLIGHT_MODE' in process.env, false);
    assert.equal(process.env.RECOMMENDATION_ML_SHADOW_ENABLED, '');
    assert.equal(process.env.RECOMMENDATION_ML_MATERIAL_SERVING_ENABLED, 'true');
    assert.equal(process.env.RECOMMENDATION_ML_MATERIAL_ARTIFACT_PATH, '/tmp/material-artifact.json');
  } finally {
...
isolatedRecommendationTest('process.env keys restore after failure', async () => {
  const originalProcess = captureRecommendationProcessEnv();
  try {
...
isolatedRecommendationTest('ML artifact cache and injection state do not leak', async () => {
  const original = captureRecommendationFlags(env);
  try {
    ...
    resetAllRecommendationTestStateForTests();
    assert.deepEqual(getMlArtifactCacheStatsForTests(), { entries: 0, artifactLoadCount: 0 });
...
if (hasDatabase) {
  isolatedRecommendationTest('parallel recommendation tests share one process isolate', async () => {
    const probe = readRecommendationTestIsolateProbe('recommendations-demo-preflight.test.ts');
  ...
  isolatedRecommendationTest('preflight then ml-shadow e2e order passes', async () => {
    ...
    const { runMlShadowComparison } = await import('../src/modules/recommendations/ml-shadow.service.js');
  ...
  isolatedRecommendationTest('ml-shadow e2e then preflight order passes', async () => {
    const flagsBefore = captureRecommendationFlags(env);
    const { runMlShadowComparison } = await import('../src/modules/recommendations/ml-shadow.service.js');
  ...
  isolatedRecommendationTest('repeated preflight and e2e sequences remain stable', async () => {
...
if (hasDatabase) {
  isolatedRecommendationTest('deterministic preflight invokes no scorer', async () => {
  ...
  isolatedRecommendationTest('material mode validates material serving only', async () => {
    const { report } = await runPreflight({
      mode: 'material',
      repositoryRoot,
      skipDisconnect: true,
    ...
    if (report.checks.artifact_versions?.ok && report.checks.project_catalog_readiness?.ok) {
      assert.equal(report.checks.material_serving_smoke?.domain, 'material');
    }
  });

  isolatedRecommendationTest('project mode requires project readiness READY', async () => {
    const { report } = await runPreflight({
      mode: 'project',
      repositoryRoot,
      skipDisconnect: true,
    ...
    if (report.checks.project_catalog_readiness?.status === 'READY') {
      assert.equal(report.checks.project_serving_smoke?.projectReadinessStatus, 'READY');
    }
  });

  isolatedRecommendationTest('both mode validates independent material and project serving', async () => {
    const { report } = await runPreflight({
      mode: 'both',
      repositoryRoot,
      skipDisconnect: true,
  ...
  test('smoke timeout report marks failure without mutating env', () => {
    const report = buildPreflightReport(
      'material',
      {
        ...
        material_serving_smoke: { ok: true, domain: 'material', status: 'SCORED' },
      },
      { flagsRestored: true, artifactPathsRestored: true },
    );
    assert.equal(report.passed, false);
  ...
  isolatedRecommendationTest('no ephemeral fixture records remain', async () => {
  ...
  isolatedRecommendationTest('accepted artifact versions are enforced', async () => {
    const paths = resolveArtifactPaths(repositoryRoot);
    const { report } = await runPreflight({
      mode: 'deterministic',
      repositoryRoot,
...
test('preflight modes cover all accepted CLI values', () => {
  assert.deepEqual(PREFLIGHT_MODES, ['deterministic', 'material', 'project', 'both']);
  assert.deepEqual([...RECOMMENDATION_PROCESS_ENV_KEYS], [
    'PREFLIGHT_MODE',
...
test('existing recommendation runtime behavior remains unchanged by preflight imports', async () => {
  const runtimeModule = await import('../src/modules/recommendations/ml-shadow.service.js');
