...
export type InteractionOrigin = 'REAL_USER' | 'DEMO_SEED' | 'TEST_FIXTURE' | 'BENCHMARK' | 'UNKNOWN';
export type EntityType = 'MATERIAL' | 'PROJECT';
export type SignalKind = 'VIEW' | 'LIKE' | 'UNLIKE' | 'SAVE' | 'UNSAVE' | 'FOLLOW' | 'UNFOLLOW' | 'RESERVATION_CREATED' | 'RESERVATION_ACCEPTED' | 'RESERVATION_COMPLETED' | 'RESERVATION_CANCELLED' | 'RESERVATION_REJECTED' | 'RESERVATION_EXPIRED' | 'RESERVATION_FAILED' | 'PROJECT_BUILD_STARTED' | 'PROJECT_BUILD_PROGRESS' | 'PROJECT_BUILD_COMPLETED' | 'IMPRESSION' | 'RECOMMENDATION_ACTION';
export type RawInteraction = { id: string; userId: string | null; itemId: string | null; entityType: EntityType | null; signal: SignalKind; state: string | null; occurredAt: Date; source: string; eventSource?: string | null; attributionType?: 'DIRECT' | 'ASSISTED' | null; operationId?: string | null; origin: InteractionOrigin };
export type ResolvedInteraction = RawInteraction & { confidence: number; eligible: boolean; reversal: boolean; dedupeKey: string; reason: string };
export const MAX_PROFILE_ROWS = 250_000;
export const VIEW_CAP_PER_USER_ITEM_DAY = 3;

const SEED_USER_EMAILS = new Set(['majd@learner.com', 'israa@learner.com', 'learner@learner.com', 'seed-learner-ahmad@impactloop.test', 'seed-learner-sara@impactloop.test', 'seed-learner-omar@impactloop.test', 'seed-learner-lina@impactloop.test', 'seed-learner-yousef@impactloop.test', 'seed-supplier@impactloop.test', 'majd@supplier.com', 'israa@supplier.com', 'supplier@supplier.com']);
const hasMarker = (value: unknown, markers: readonly string[]) => markers.some((marker) => String(value ?? '').toLowerCase().includes(marker));

export const classifyOrigin = (input: { eventSource?: string | null; userEmail?: string | null; itemText?: string | null; sourceText?: string | null }): InteractionOrigin => {
  ...
  if (hasMarker(input.userEmail, ['test-', 'test_', 'fixture', 'benchmark', 'load-test']) || hasMarker(input.itemText, ['[test-', 'test-internal', 'fixture']) || hasMarker(input.sourceText, ['[test-', 'fixture'])) return 'TEST_FIXTURE';
  if (SEED_USER_EMAILS.has((input.userEmail ?? '').toLowerCase()) || hasMarker(input.itemText, ['[realistic-impactloop-seed]', '(spare batch)']) || hasMarker(input.sourceText, ['viewsource:seed', 'seed reservation'])) return 'DEMO_SEED';
  if (eventSource === 'REAL' || input.userEmail) return 'REAL_USER';
  return 'UNKNOWN';
};

const utcDay = (date: Date) => date.toISOString().slice(0, 10);
const reversal = (signal: SignalKind) => ['UNLIKE', 'UNSAVE', 'UNFOLLOW'].includes(signal);
const positive = (signal: SignalKind) => !['UNLIKE', 'UNSAVE', 'UNFOLLOW', 'IMPRESSION', 'RECOMMENDATION_ACTION', 'RESERVATION_CANCELLED', 'RESERVATION_REJECTED', 'RESERVATION_EXPIRED', 'RESERVATION_FAILED'].includes(signal);
const confidenceFor = (signal: SignalKind) => ({ VIEW: 0.25, LIKE: 2, SAVE: 3, FOLLOW: 2.5, RESERVATION_CREATED: 4, RESERVATION_ACCEPTED: 5, RESERVATION_COMPLETED: 6, PROJECT_BUILD_STARTED: 4, PROJECT_BUILD_PROGRESS: 5, PROJECT_BUILD_COMPLETED: 6, RESERVATION_CANCELLED: -1, RESERVATION_REJECTED: -1, RESERVATION_EXPIRED: -1, RESERVATION_FAILED: -1, UNLIKE: 0, UNSAVE: 0, UNFOLLOW: 0 } as Partial<Record<SignalKind, number>>)[signal] ?? 0;
const actionSignal = (action: string): SignalKind => ({ MATERIAL_VIEW: 'VIEW', MATERIAL_LIKE: 'LIKE', MATERIAL_UNLIKE: 'UNLIKE', RESERVATION_CREATED: 'RESERVATION_CREATED', RESERVATION_SUBMITTED: 'RESERVATION_CREATED', PROJECT_LIKE: 'LIKE', PROJECT_UNLIKE: 'UNLIKE', PROJECT_SAVE: 'SAVE', PROJECT_UNSAVE: 'UNSAVE', PROJECT_FOLLOW: 'FOLLOW', PROJECT_UNFOLLOW: 'UNFOLLOW', PROJECT_BUILD_STARTED: 'PROJECT_BUILD_STARTED', PROJECT_BUILD_PROGRESS_UPDATED: 'PROJECT_BUILD_PROGRESS', PROJECT_BUILD_PROGRESSED: 'PROJECT_BUILD_PROGRESS' } as Record<string, SignalKind>)[action] ?? 'RECOMMENDATION_ACTION';

export const resolveInteractionRecords = (raw: RawInteraction[]): ResolvedInteraction[] => {
  const ordered = [...raw].sort((a, b) => a.occurredAt.getTime() - b.occurredAt.getTime() || a.id.localeCompare(b.id));
  ...
  for (const row of ordered) {
    ...
    if (row.signal === 'VIEW') {
      ...
      output.push({ ...row, confidence: count ? 0.5 : 0.25, eligible: true, reversal: false, dedupeKey: `${row.id}|${key}`, reason: count ? 'capped repeat view' : 'first view in UTC day' });
      continue;
    }
    if (['LIKE', 'UNLIKE', 'SAVE', 'UNSAVE', 'FOLLOW', 'UNFOLLOW'].includes(row.signal)) {
      const family = row.signal === 'LIKE' || row.signal === 'UNLIKE' ? 'LIKE' : row.signal === 'SAVE' || row.signal === 'UNSAVE' ? 'SAVE' : 'FOLLOW';
      const key = `${pair}|${family}`;
      stateEvents.set(key, [...(stateEvents.get(key) ?? []), row]);
      continue;
    }
    const operationKey = row.operationId ? `${row.operationId}|${row.signal}` : null;
    ...
    output.push({ ...row, confidence: score, eligible: positive(row.signal) && score > 0, reversal: false, dedupeKey: row.operationId ?? row.id, reason: row.state ?? row.signal });
  }
  for (const [key, rows] of stateEvents) {
    const final = [...rows].sort((a, b) => b.occurredAt.getTime() - a.occurredAt.getTime() || b.id.localeCompare(a.id))[0]!;
    const active = !reversal(final.signal);
    output.push({ ...final, confidence: active ? confidenceFor(final.signal) : 0, eligible: active, reversal: !active, dedupeKey: `${key}|active-state`, reason: active ? 'latest active state' : 'latest reversal removes active state' });
  }
  return output.sort((a, b) => a.occurredAt.getTime() - b.occurredAt.getTime() || a.dedupeKey.localeCompare(b.dedupeKey));
};

export const hasOperationLeakage = (train: RawInteraction[], test: RawInteraction[]) => {
...
export const buildGraphMetrics = (pairs: Pair[]): GraphMetrics => {
  const userItems = new Map<string, Set<string>>(); const itemUsers = new Map<string, Set<string>>(); const adjacency = new Map<string, Set<string>>();
  for (const pair of pairs) { if (!userItems.has(pair.userId)) userItems.set(pair.userId, new Set()); if (!itemUsers.has(pair.itemId)) itemUsers.set(pair.itemId, new Set()); userItems.get(pair.userId)!.add(pair.itemId); itemUsers.get(pair.itemId)!.add(pair.userId); const user = `u:${pair.userId}`, item = `i:${pair.itemId}`; if (!adjacency.has(user)) adjacency.set(user, new Set()); if (!adjacency.has(item)) adjacency.set(item, new Set()); adjacency.get(user)!.add(item); adjacency.get(item)!.add(user); }
  const nodes = [...adjacency.keys()]; const seen = new Set<string>(); const sizes: number[] = [];
  for (const node of nodes) { if (seen.has(node)) continue; const queue = [node]; seen.add(node); let size = 0; while (queue.length) { const current = queue.shift()!; size++; for (const next of adjacency.get(current) ?? []) if (!seen.has(next)) { seen.add(next); queue.push(next); } } sizes.push(size); }
  const overlaps = (sets: Map<string, Set<string>>) => { const keys = [...sets.keys()].sort(); const result: Array<{ left: string; right: string; count: number }> = []; for (let i = 0; i < keys.length; i++) for (let j = i + 1; j < keys.length; j++) { let count = 0; for (const value of sets.get(keys[i]!)!) if (sets.get(keys[j]!)!.has(value)) count++; if (count) result.push({ left: keys[i]!, right: keys[j]!, count }); } return result.sort((a, b) => b.count - a.count || `${a.left}|${a.right}`.localeCompare(`${b.left}|${b.right}`)).slice(0, 20); };
  const totalNodes = userItems.size + itemUsers.size;
  return { connectedComponents: sizes.length, largestComponentShare: totalNodes ? Math.max(...sizes, 0) / totalNodes : 0, isolatedUsers: 0, isolatedItems: 0, userDegree: summarizeDistribution([...userItems.values()].map((set) => set.size)), itemDegree: summarizeDistribution([...itemUsers.values()].map((set) => set.size)), itemPairCooccurrence: overlaps(itemUsers), userPairOverlap: overlaps(userItems) };
};

const pseudonymizeGraph = (graph: GraphMetrics): GraphMetrics => {
  const pseudonym = (value: string) => createHash('sha256').update(value).digest('hex').slice(0, 16);
  return {
    ...graph,
    itemPairCooccurrence: graph.itemPairCooccurrence.map((pair) => ({ ...pair, left: pseudonym(pair.left), right: pseudonym(pair.right) })),
    userPairOverlap: graph.userPairOverlap.map((pair) => ({ ...pair, left: pseudonym(pair.left), right: pseudonym(pair.right) })),
  };
};

const bounded = async <T>(rows: Promise<T[]>, total: number) => ({ rows: await rows, total, capped: total > MAX_PROFILE_ROWS });
const statusSignal = (status: string): SignalKind => status === 'COMPLETED' ? 'RESERVATION_COMPLETED' : status === 'ACCEPTED' ? 'RESERVATION_ACCEPTED' : status === 'CANCELLED' ? 'RESERVATION_CANCELLED' : status === 'REJECTED' ? 'RESERVATION_REJECTED' : status === 'EXPIRED' ? 'RESERVATION_EXPIRED' : ['NO_SHOW', 'FULFILLMENT_FAILED'].includes(status) ? 'RESERVATION_FAILED' : 'RESERVATION_CREATED';

export async function profileDatabase(client: PrismaClient = prisma) {
  const started = performance.now();
  const [users, materials, projects] = await Promise.all([
    client.user.findMany({ where: { roles: { some: { role: 'LEARNER' } } }, select: { id: true, email: true, accountStatus: true } }),
  ...
  const [views, likes, reservations, histories, projectLikes, saves, follows, builds, impressions, actions] = await Promise.all([
    ...
    bounded(client.reservation.findMany({ take: MAX_PROFILE_ROWS, select: { id: true, materialId: true, requesterId: true, status: true, createdAt: true, updatedAt: true, acceptedAt: true, rejectedAt: true, cancelledAt: true, completedAt: true, message: true, material: { select: { title: true, description: true } }, requester: { select: { email: true } } } }), reservationCount),
    bounded(client.reservationStatusHistory.findMany({ take: MAX_PROFILE_ROWS, select: { id: true, reservationId: true, oldStatus: true, newStatus: true, createdAt: true } }), historyCount),
    bounded(client.projectLike.findMany({ take: MAX_PROFILE_ROWS, select: { id: true, projectId: true, userId: true, createdAt: true, project: { select: { title: true } }, user: { select: { email: true } } } }), projectLikeCount),
    bounded(client.projectSave.findMany({ take: MAX_PROFILE_ROWS, select: { id: true, projectId: true, userId: true, createdAt: true, project: { select: { title: true } }, user: { select: { email: true } } } }), saveCount),
  ...
  for (const row of views.rows) add({ id: row.id, userId: row.viewerUserId, itemId: row.materialId, entityType: 'MATERIAL', signal: 'VIEW', state: null, occurredAt: row.createdAt, source: row.viewSource ?? 'material_views', origin: classifyOrigin({ userEmail: userById.get(row.viewerUserId ?? '')?.email, itemText: `${row.material.title} ${row.material.description}`, sourceText: row.viewSource }) });
  for (const row of likes.rows) add({ id: row.id, userId: row.userId, itemId: row.materialId, entityType: 'MATERIAL', signal: 'LIKE', state: 'ACTIVE', occurredAt: row.createdAt, source: 'material_likes', origin: classifyOrigin({ userEmail: row.user.email, itemText: `${row.material.title} ${row.material.description}` }) });
  for (const row of reservations.rows) add({ id: row.id, userId: row.requesterId, itemId: row.materialId, entityType: 'MATERIAL', signal: statusSignal(row.status), state: row.status, occurredAt: row.createdAt, source: 'reservations', operationId: row.id, origin: classifyOrigin({ userEmail: row.requester.email, itemText: `${row.material.title} ${row.material.description}`, sourceText: row.message }) });
  for (const row of projectLikes.rows) add({ id: row.id, userId: row.userId, itemId: row.projectId, entityType: 'PROJECT', signal: 'LIKE', state: 'ACTIVE', occurredAt: row.createdAt, source: 'project_likes', origin: classifyOrigin({ userEmail: row.user.email, itemText: row.project.title }) });
  for (const row of saves.rows) add({ id: row.id, userId: row.userId, itemId: row.projectId, entityType: 'PROJECT', signal: 'SAVE', state: 'ACTIVE', occurredAt: row.createdAt, source: 'project_saves', origin: classifyOrigin({ userEmail: row.user.email, itemText: row.project.title }) });
  for (const row of follows.rows) add({ id: row.id, userId: row.userId, itemId: row.projectId, entityType: 'PROJECT', signal: 'FOLLOW', state: 'ACTIVE', occurredAt: row.createdAt, source: 'project_follows', origin: classifyOrigin({ userEmail: row.user.email, itemText: row.project.title }) });
  for (const row of builds.rows) { const origin = classifyOrigin({ userEmail: row.learner.email, itemText: row.project.title }); add({ id: `${row.id}:start`, userId: row.learnerId, itemId: row.projectId, entityType: 'PROJECT', signal: 'PROJECT_BUILD_STARTED', state: row.status, occurredAt: row.startedAt, source: 'project_builds', operationId: row.id, origin }); for (const item of row.items.filter((item) => item.status !== 'MISSING')) add({ id: `${row.id}:item:${item.id}`, userId: row.learnerId, itemId: row.projectId, entityType: 'PROJECT', signal: row.status === 'COMPLETED' ? 'PROJECT_BUILD_COMPLETED' : 'PROJECT_BUILD_PROGRESS', state: item.status, occurredAt: item.updatedAt, source: 'project_build_items', operationId: `${row.id}:${item.id}`, origin }); }
  for (const row of impressions.rows) add({ id: row.id, userId: row.learnerId, itemId: row.entityId, entityType: row.entityType, signal: 'IMPRESSION', state: null, occurredAt: row.shownAt, source: 'recommendation_impressions', eventSource: row.eventSource, origin: classifyOrigin({ eventSource: row.eventSource, userEmail: row.learner.email }) });
  ...
  const materialRealUsers = new Set(pairs('MATERIAL').map((pair) => pair.userId)); const projectRealUsers = new Set(pairs('PROJECT').map((pair) => pair.userId)); const allUsers = [...new Set(allObserved.map((row) => row.userId!))]; const materialAllUsers = new Set(allPairs('MATERIAL').map((pair) => pair.userId)); const projectAllUsers = new Set(allPairs('PROJECT').map((pair) => pair.userId)); const originDistributions = Object.fromEntries((['REAL_USER', 'DEMO_SEED', 'TEST_FIXTURE', 'BENCHMARK', 'UNKNOWN'] as InteractionOrigin[]).map((origin) => { const rows = allObserved.filter((row) => row.origin === origin); const groups = grouped(rows, (row) => row.userId!); return [origin, { rows: rows.length, users: groups.length, usersWithAtLeast2UniqueItems: usersAtLeast(groups, 2), usersWithAtLeast5UniqueItems: usersAtLeast(groups, 5), usersWithAtLeast10UniqueItems: usersAtLeast(groups, 10), interactionsPerUser: dist(groups), uniqueItemsPerUser: dist(groups, true) }]; })); const profile = {
    ...
    distributions: { interactionsPerUser: dist(userGroups), uniqueItemsPerUser: dist(userGroups, true), interactionsPerMaterial: dist(materialGroups), interactionsPerProject: dist(projectGroups), activityDaysPerUser: summarizeDistribution([...daysByUser.values()].map((set) => set.size)), activeTimeSpanDays: first && last ? Math.ceil((last.getTime() - first.getTime()) / 86_400_000) : 0, repeatInteractionRate: real.length ? 1 - new Set(real.map((row) => `${row.userId}|${row.entityType}|${row.itemId}`)).size / real.length : 0 },
    allCurrentRows: { interactionsPerUser: dist(grouped(allObserved, (row) => row.userId!)), uniqueItemsPerUser: dist(grouped(allObserved, (row) => row.userId!), true), interactionsPerMaterial: dist(materials.map((item) => allObserved.filter((row) => row.entityType === 'MATERIAL' && row.itemId === item.id))), interactionsPerProject: dist(projects.map((item) => allObserved.filter((row) => row.entityType === 'PROJECT' && row.itemId === item.id))) },
    matrices: { material: buildMatrixMetrics(activeUsers, materialIds, pairs('MATERIAL')), project: buildMatrixMetrics(activeUsers, projectIds, pairs('PROJECT')), combined: buildMatrixMetrics(activeUsers, [...materialIds.map((id) => `MATERIAL:${id}`), ...projectIds.map((id) => `PROJECT:${id}`)], real.map((row) => ({ userId: row.userId!, itemId: `${row.entityType}:${row.itemId}`, occurredAt: row.occurredAt }))), allCurrentRows: { material: buildMatrixMetrics(allUsers, materials.map((item) => item.id), allPairs('MATERIAL')), project: buildMatrixMetrics(allUsers, projects.map((item) => item.id), allPairs('PROJECT')) } },
