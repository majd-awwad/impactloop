...
const markdownOutputPath = path.join(outputDir, 'slice-4j-a-evaluation-summary.md');

const priorFlags = captureRecommendationFlags(env);
const allObservations: Array<ShadowDiagnostics & { domain: 'material' | 'project' }> = [];
...
const parseCliOptions = () => {
  const modeArg = process.argv.find((argument) => argument.startsWith('--mode='))?.slice('--mode='.length);
  const latencyOnly = process.argv.includes('--latency-only');
  const modeFilter =
    modeArg && modeArg in EVALUATION_MODES ? (modeArg as EvaluationModeKey) : undefined;
...
const probeArchetypes = async (
  ...
  for (const selection of selections) {
    ...
    const { observations } = await collectRequestObservations(observationsForRun, async () => {
...
const runLearnerCase = async (input: {
  ...
  try {
    ...
    httpStatus = 500;
    throw new Error(`learner_home_failed:${input.archetypeKey}:${input.modeKey}`);
  }
  ...
  const materialDiagnostics = observationsForRun.find((value) => value.domain === 'material');
  const projectDiagnostics = observationsForRun.find((value) => value.domain === 'project');
  ...
  if (metrics.unavailableMaterialCount > 0 || metrics.ineligibleCandidateCount > 0) {
...
const discoverSparseCandidateLearner = async (learners: LearnerFixtureRow[]) => {
  applyRecommendationFlags(env, flagsForMode('B_shadow'));
  clearMlArtifactCacheForTests();
  ...
  for (const learner of learners.slice(0, 10)) {
    ...
    const candidateCount = Math.min(
      observationsForRun.find((value) => value.domain === 'material')?.candidateCount ??
        Number.MAX_SAFE_INTEGER,
      observationsForRun.find((value) => value.domain === 'project')?.runtimeCandidateCount ??
        observationsForRun.find((value) => value.domain === 'project')?.candidateCount ??
        Number.MAX_SAFE_INTEGER,
    );
...
const runFallbackIndependence = async (input: {
  ...
  const modeDProjects = extractSectionProjectIds(modeDValid, 'suggested_projects').slice(0, 4);
  ...
  try {
    ...
    materialFallbackProjectUnchanged =
      ...
      JSON.stringify(
        extractSectionProjectIds(modeDMaterialBroken, 'suggested_projects').slice(0, 4),
      );
  } catch {
  ...
  const modeCMaterials = extractSectionMaterialIds(modeCValid, 'suggested_materials').slice(0, 4);
  ...
  try {
    ...
    projectFallbackMaterialUnchanged =
      ...
      JSON.stringify(
        extractSectionMaterialIds(modeCProjectBroken, 'suggested_materials').slice(0, 4),
      );
  } catch {
...
const runLatencyBenchmark = async (
  ...
  const modes = (modeFilter
    ? [modeFilter]
    : Object.keys(EVALUATION_MODES)) as EvaluationModeKey[];
  ...
  for (const modeKey of modes) {
    ...
    const coldSample = await collectRequestObservations(observationsForRun, async () => {
    ...
    for (let index = 0; index < 8; index += 1) {
      ...
      const warmSample = await collectRequestObservations(observationsForRun, async () => {
...
const runBaselineOnly = async (resolvedArchetypes: Array<{ archetypeKey: ArchetypeKey; learnerId: string }>) => {
  const latencySummary = await runLatencyBenchmark(resolvedArchetypes, 'A_baseline');
...
const buildQualityProxies = (
  ...
  const servedCases = cases.filter(
    (entry) => entry.modeKey === 'C_material' || entry.modeKey === 'D_project' || entry.modeKey === 'E_both',
  );
  const archetypeKey = servedCases[0]?.archetypeKey ?? 'mixed';
  const context = learnerContexts.get(archetypeKey);
  const materialDiagnostics = diagnostics.find((entry) => entry.domain === 'material');
  const projectDiagnostics = diagnostics.find((entry) => entry.domain === 'project');
  ...
  return {
    materials: computeMaterialQualityProxies({
      ...
      recentDomainRepresentation: materialDiagnostics?.fusedTop5RecentDomainCount ?? 0,
      longTermRecentOverlap: materialDiagnostics?.longTermRecentTop5OverlapCount ?? 0,
    ...
    projects: computeProjectQualityProxies({
      ...
      interests: context?.interests ?? [],
      recentDomainRepresentation: projectDiagnostics?.fusedTop5RecentDomainCount ?? 0,
      longTermRecentOverlap: projectDiagnostics?.longTermRecentTop5OverlapCount ?? 0,
  ...
  try {
    for (const archetype of resolvedArchetypes) {
      for (const modeKey of Object.keys(EVALUATION_MODES) as EvaluationModeKey[]) {
        ...
        if (modeKey === 'E_both' && !learnerContexts.has(archetype.archetypeKey)) {
          const profile = learners.find((learner) => learner.id === archetype.learnerId);
          learnerContexts.set(archetype.archetypeKey, {
            ...
            prefersFree: false,
            hasContinueProjects: Boolean(
              result.home.sections.find((section) => section.key === 'continue_projects')?.items
                .length,
            ),
            home: result.home,
          });
    ...
    const artifactVersions = {
      material: allObservations.find((entry) => entry.domain === 'material')?.artifactVersion ?? 'unknown',
      project: allObservations.find((entry) => entry.domain === 'project')?.artifactVersion ?? 'unknown',
    };
    const featureSchemaVersions = {
      material:
        allObservations.find((entry) => entry.domain === 'material')?.featureSchemaVersion ?? 'unknown',
      project:
        allObservations.find((entry) => entry.domain === 'project')?.featureSchemaVersion ?? 'unknown',
    };

    const confidenceDistributions = aggregateConfidenceDistribution(modeComparisons, allObservations);
    const qualityProxies = buildQualityProxies(modeComparisons, allObservations, learnerContexts);
    const fallbackSummary = {
      totalFallbackCases: modeComparisons.filter((entry) => entry.fallbackOccurred).length,
      byReason: modeComparisons.reduce<Record<string, number>>((accumulator, entry) => {
        const reason = entry.fallbackReasonCategory ?? 'none';
        accumulator[reason] = (accumulator[reason] ?? 0) + (entry.fallbackOccurred ? 1 : 0);
        return accumulator;
      }, {}),
    };
    ...
    const reportDraft: Slice4jAReport = {
      ...
      determinism: { stableHash: '', excludes: ['generatedAt', 'requestDurationMs', 'latencySummary'] },
    };
    ...
    await writeFile(jsonOutputPath, `${JSON.stringify(envelope, null, 2)}\n`, 'utf8');
