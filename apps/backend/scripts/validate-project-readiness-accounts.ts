...
const coherent = learners.find((learner) => {
  ...
  return (
    projectIds.size >= 2 &&
    learner._count.projectSaves + learner._count.projectLikes >= 3
  );
});
...
const cases = [
  ...
  {
    label: "scattered",
    learner: scattered ?? learners.find((learner) => score(learner) > 5),
  },
...
for (const entry of cases) {
  ...
  const materialDiagnostics = observations.find(
  ...
  results.push({
    case: entry.label,
    publicOrderingUnchanged:
      JSON.stringify(suggestedProjects) ===
      JSON.stringify(deterministicProjects),
    ...
    artifactMappedCandidateCount:
      projectDiagnostics?.artifactMappedCandidateCount,
    missingArtifactCandidateCount:
      projectDiagnostics?.runtimeCandidatesMissingFromArtifact,
    duplicateRuntimeCandidateCount:
      projectDiagnostics?.duplicateRuntimeCandidateCount,
