...
import {
  ...
  validateRecommendationReleaseConfigSync,
...
import {
  createRecommendationDatabaseContractCheck,
  runValidateRecommendationReleaseConfigCli,
} from "./validate-recommendation-release-config.js";
...
const validateSync = (env: NodeJS.Dict<string>) =>
  validateRecommendationReleaseConfigSync(env, { parseScorerVersion });
...
const cliModuleHref = pathToFileURL(
  path.join(backendRoot, "scripts/validate-recommendation-release-config.ts"),
).href;

/**
 * Fresh Node process that imports the real CLI entry and calls
 * runValidateRecommendationReleaseConfigCli({ loadDotenv: false, env: process.env }).
...
const runValidatorChild = (
  ...
  new Promise((resolve, reject) => {
    ...
    writeFileSync(
      harnessPath,
      `
import { runValidateRecommendationReleaseConfigCli } from ${JSON.stringify(cliModuleHref)};

const args = JSON.parse(process.env.VALIDATOR_TEST_ARGV ?? "[]");

void runValidateRecommendationReleaseConfigCli(args, {
  loadDotenv: false,
  env: process.env,
}).then((exitCode) => {
...
test("human and JSON formatting are secret-safe and useful", () => {
  ...
  const human = formatRecommendationReleaseHumanReport(result);
...
test("secret redaction removes nested error secrets from human and JSON output", () => {
  ...
  const human = formatRecommendationReleaseHumanReport(nestedResult);
...
test("secret redaction covers key/value secrets outside URLs", () => {
  ...
  assert.equal(formatRecommendationReleaseHumanReport(result).includes("super-secret-value"), false);
  assert.equal(formatJsonReport(result).includes("super-secret-value"), false);
  assert.equal(formatRecommendationReleaseHumanReport(result).includes("mail-pass"), false);
...
test("unknown CLI argument containing a secret is not echoed", async () => {
  ...
  try {
    ...
    const code = await runValidateRecommendationReleaseConfigCli([secretArg], {
      loadDotenv: false,
...
test("CLI unknown argument returns exit code 2", async () => {
  ...
  try {
    const code = await runValidateRecommendationReleaseConfigCli(["--nope"], {
      loadDotenv: false,
      env: validReleaseAEnv(),
    });
    assert.equal(code, 2);
...
test("CLI --json --unexpected emits JSON and exit code 2", async () => {
  ...
  try {
    const code = await runValidateRecommendationReleaseConfigCli(
      ["--json", "--unexpected"],
      {
        loadDotenv: false,
        env: validReleaseAEnv(),
...
test(
  ...
  async () => {
    ...
    const { database, cleanup } = createRecommendationDatabaseContractCheck(connectionString);
    try {
