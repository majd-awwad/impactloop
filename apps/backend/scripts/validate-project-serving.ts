...
const projectArtifact = path.join(
  root,
...
const profileOnly = learners.find(
  (learner) =>
    (learner.learnerProfile?.interests.length ?? 0) > 0 && score(learner) === 0,
);
const activeLearner =
  learners.find((learner) => score(learner) > 2) ?? learners[0];

const results: Array<Record<string, unknown>> = [];

for (const [label, learner] of [
  ["profile-only", profileOnly],
  ...
  const deterministicProjects =
    deterministic.sections
      .find((section) => section.key === "suggested_projects")
      ?.items.map((item) => (item.type === "project" ? item.project.id : "")) ??
    [];
  env.recommendationMlProjectServingEnabled = true;
  invalidateLearnerHomeCache(learner.id);
  observations.length = 0;
  ...
  const servedProjects =
    served.sections
      ...
      ?.items.map((item) => (item.type === "project" ? item.project.id : "")) ??
    [];
  const section = await getLearnerHomeSection(
    learner.id,
    "suggested_projects",
    4,
  );
  const sectionProjects = section.items
    .filter((item) => item.type === "project")
    .map((item) => item.project.id);
  const projectDiagnostics = observations.find(
  ...
  results.push({
    ...
    totalProjectRecommendationDurationMs:
      projectDiagnostics?.totalProjectRecommendationDurationMs,
    homeSectionConsistent:
      JSON.stringify(servedProjects.slice(0, 4)) ===
      JSON.stringify(sectionProjects),
    materialSectionsUnchanged:
      JSON.stringify(
        served.sections
          .filter((value) => value.key !== "suggested_projects")
          .map((value) => value.key),
      ) ===
      JSON.stringify(
        deterministic.sections
    ...
    suggestedProjectsChanged:
      JSON.stringify(deterministicProjects) !== JSON.stringify(servedProjects),
  });
}
