...
import {
  type AuditInvariantResult,
  type AuditProfile,
  type NPlusOneContext,
...
const resolveLearnerId = async (email: string) => {
...
const timeCall = async <T>(run: () => Promise<T>) => {
...
const invariant = (
  name: string,
  pass: boolean,
  expected: AuditInvariantResult['expected'],
  observed: AuditInvariantResult['observed'],
  evidence: string,
...
const knownTableMisclassifiedAsOther = (
...
const runMeasuredAuditBody = async (options: AuditCliOptions) => {
  ...
  invariants.push(
    invariant(
      'query_events_enabled',
      queryEventsEnabled,
      true,
      queryEventsEnabled,
      'PRISMA_QUERY_EVENTS must be 1 for SQL measurement',
    ),
  );

  const unverifiedRequired = callerMatrix.filter(
    (row) => row.required && row.classification === 'UNVERIFIED_CALLER',
  );
  invariants.push(
    invariant(
      'required_invalidation_callers_verified',
      unverifiedRequired.length === 0,
      0,
      unverifiedRequired.length,
      unverifiedRequired.map((row) => row.mutation).join('; ') ||
  ...
  type ColdSample = {
    durationMs: number;
    cacheState: string;
    querySummary: ReturnType<typeof summarizeLearnerHomeQueryEvents>;
    candidateCount: number;
    events: LearnerHomeQueryEvent[];
    requestedMaterialScoringMode: string | null;
    effectiveMaterialScoringMode: string | null;
  };

  const coldSamples: ColdSample[] = [];
  ...
  for (let index = 0; index < options.coldSamples; index += 1) {
    ...
    const sample = await withFrozenEvaluationTime(evaluationTimeUtc, async () => {
      if (!queryEventsEnabled) {
        const timed = await timeCall(() =>
          getLearnerHomeWithCacheStateForAudit(learner.id),
        );
        return {
          ...
          events: [] as LearnerHomeQueryEvent[],
          requestedMaterialScoringMode:
            timed.result.modeDecision.requestedMaterialScoringMode,
          effectiveMaterialScoringMode:
            timed.result.modeDecision.effectiveMaterialScoringMode,
        };
      }
      const measured = await measureWithQueryAudit(async () =>
        timeCall(() => getLearnerHomeWithCacheStateForAudit(learner.id)),
      );
      return {
        ...
        requestedMaterialScoringMode:
          measured.result.result.modeDecision.requestedMaterialScoringMode,
        effectiveMaterialScoringMode:
          measured.result.result.modeDecision.effectiveMaterialScoringMode,
      };
    });
  ...
  invariants.push(
    invariant(
      'full_home_cold_is_miss',
      coldMissCheck.pass,
      'MISS',
      coldMissCheck.pass
        ? 'MISS'
        : coldSamples.map((sample) => sample.cacheState).join(','),
      coldMissCheck.failingIndexes.length > 0
  ...
  invariants.push(
    invariant(
      ...
      resolveMaterialCandidatePoolCap({ kind: 'FULL_HOME' }) ===
        HOME_MATERIAL_POOL_CAP && HOME_MATERIAL_POOL_CAP === 120,
      120,
      resolveMaterialCandidatePoolCap({ kind: 'FULL_HOME' }),
      'resolveMaterialCandidatePoolCap FULL_HOME',
    ),
  );
  ...
  if (queryEventsEnabled) {
    invariants.push(
      invariant(
        'cold_known_tables_not_other',
        !knownTableMisclassifiedAsOther(lastCold.events),
        0,
        lastCold.events.filter(
          (event) => classifyLearnerHomeQueryEvent(event) === 'Other',
        ).length,
        'mapped-table SQL classifier on Full Home miss',
      ),
    );
  }
  ...
  invariants.push(
    invariant(
      'full_home_warm_is_hit',
      warmHitCheck.pass,
      'HIT',
      warmHitCheck.pass
        ? 'HIT'
        : warmSamples.map((sample) => sample.cacheState).join(','),
      warmHitCheck.failingIndexes.length > 0
  ...
  invariants.push(
    invariant(
      'warm_hit_no_candidate_loading_sql',
      warmSqlCheck.pass,
      0,
      warmSqlCheck.failingIndexes.length,
      warmSqlCheck.evidence,
    ),
  );
  ...
  type SectionMeasurement = {
    ...
    candidateCounts: { materials: number; projects: number };
    appliedPoolCap: number;
    requestedMaterialScoringMode: string;
    effectiveMaterialScoringMode: string;
    fallbackCode: string | null;
    loadCanonicalMaterialConceptsMs: number | null;
    materialConceptSql: number;
  };

  let suggestedMaterials: SectionMeasurement | null = null;
  let freeMaterialsNearYou: SectionMeasurement | null = null;
  let suggestedProjects:
    | (SectionMeasurement & {
        materialOverfetch: number;
        materialConceptAttribution: ReturnType<
          typeof attributeProjectOnlyMaterialConceptSql
        >;
      })
    | null = null;
  ...
  const measureSection = async (
    ...
    withFrozenEvaluationTime(evaluationTimeUtc, async () => {
      const measured = await measureWithQueryAudit(async () =>
        timeCall(() =>
          getLearnerHomeSectionForAudit(learner.id, sectionKey, 4, 0),
        ),
      );
      ...
      return {
        ...
        loadCanonicalMaterialConceptsMs:
          typeof audited.timingsMs?.loadCanonicalMaterialConcepts === 'number'
  ...
  if (queryEventsEnabled) {
    ...
    const materialConceptAttribution = attributeProjectOnlyMaterialConceptSql({
      ...
      loadCanonicalMaterialConceptsMs:
        projectsSection.loadCanonicalMaterialConceptsMs,
    ...
    invariants.push(
      invariant(
        'browse_cap_400',
        suggestedMaterials.appliedPoolCap === BROWSE_MATERIAL_POOL_CAP &&
          BROWSE_MATERIAL_POOL_CAP === 400,
        400,
        suggestedMaterials.appliedPoolCap,
        'resolveMaterialCandidatePoolCap suggested_materials',
      ),
    );
    invariants.push(
      invariant(
        'non_browse_material_cap_120',
        freeMaterialsNearYou.appliedPoolCap === HOME_MATERIAL_POOL_CAP,
        120,
        freeMaterialsNearYou.appliedPoolCap,
        'resolveMaterialCandidatePoolCap free_materials_near_you',
      ),
    );
  }

  invalidateLearnerHomeCache(learner.id);
  const hydrationAudit = await withFrozenEvaluationTime(
    evaluationTimeUtc,
    async () => runLearnerHomeUncachedAudit(learner.id),
  );

  const loadCanonicalMaterialConceptsMs =
    typeof hydrationAudit.timingsMs.loadCanonicalMaterialConcepts === 'number'
  ...
  for (const key of requiredTimingKeys) {
    invariants.push(
      invariant(
        `timing_present_${key}`,
        typeof hydrationAudit.timingsMs[key] === 'number',
        true,
        typeof hydrationAudit.timingsMs[key] === 'number',
        'runLearnerHomeUncachedAudit profiler.getTimings()',
      ),
    );
  }
  ...
  invariants.push(
    ...evaluateCanonicalHydrationRequirement({
      ...
      requestedMaterialScoringMode:
        hydrationAudit.requestedMaterialScoringMode,
      effectiveMaterialScoringMode:
        hydrationAudit.effectiveMaterialScoringMode,
  ...
  if (suggestedProjects) {
    ...
    const canonicalZero =
      attribution.findings.find(
        (row) => row.code === 'CANONICAL_PROJECT_ONLY_ZERO_HYDRATION',
      )!;
    invariants.push(
      invariant(
        'CANONICAL_PROJECT_ONLY_ZERO_HYDRATION',
        canonicalZero.status === 'PASS',
        0,
        attribution.canonicalMaterialConceptSql,
        canonicalZero.evidence,
      ),
    );
    ...
    if (profile === 'canonical-isolated') {
      invariants.push(
        invariant(
          'canonical_isolated_project_only_zero_material_concept_sql',
          attribution.totalMaterialConceptSql === 0,
          0,
          attribution.totalMaterialConceptSql,
          'ML flags disabled for canonical-isolated; project-only must have zero MaterialConcept SQL',
        ),
      );
    }
  }
  ...
  let chunkBoundStatus: 'PASS' | 'N_PLUS_ONE_UNVERIFIED' | 'FAIL' =
    'N_PLUS_ONE_UNVERIFIED';
  let chunkBoundEvidence = 'query events disabled';
  if (queryEventsEnabled) {
    const { loadMaterialConceptsForScoring } = await import(
      '../src/modules/learner-home/learner-home.repository.js'
    );
    const ids50 = Array.from({ length: 50 }, (_, index) => `rp035-n1-50-${index}`);
    const ids250 = Array.from(
      { length: 250 },
      (_, index) => `rp035-n1-250-${index}`,
    );
    const measured50 = await measureWithQueryAudit(async () =>
      loadMaterialConceptsForScoring(ids50),
    );
    const measured250 = await measureWithQueryAudit(async () =>
      loadMaterialConceptsForScoring(ids250),
    );
  ...
  const measureCandidateFixture = async (
    label: string,
    materialPoolCap: number,
    projectTake: number,
  ): Promise<NPlusOneContext> => {
    const measured = await measureWithQueryAudit(async () => {
      ...
      return {
        ...
        requiredComponentCount: projects.reduce(
  ...
  let broaderNPlusOne = {
    status: 'N_PLUS_ONE_UNVERIFIED' as const,
    evidence: 'query events disabled',
    shapeComparisons: [] as ReturnType<typeof evaluateBroaderNPlusOne>['shapeComparisons'],
    violations: [] as ReturnType<typeof evaluateBroaderNPlusOne>['violations'],
  };
  if (queryEventsEnabled) {
    const smallCtx = await measureCandidateFixture(
      'repo-fixture-small',
      20,
      8,
    );
    const largeCtx = await measureCandidateFixture(
      'repo-fixture-large',
      HOME_MATERIAL_POOL_CAP,
      40,
    );
      ...
      const coldCheck = evaluateBroaderNPlusOne({
        ...
        large: {
          ...
          requiredComponentCount:
            hydrationAudit.candidateCounts.requiredComponents ??
  ...
  invariants.push(
    invariant(
      'broader_n_plus_one',
      broaderNPlusOne.status === 'PASS',
      'PASS',
      broaderNPlusOne.status,
      broaderNPlusOne.evidence,
    ),
  );
  ...
  const sectionReport = (section: SectionMeasurement | null) =>
    section
      ? {
          ...
          loadCanonicalMaterialConceptsMs:
            section.loadCanonicalMaterialConceptsMs,
    ...
    measurements: {
      ...
      suggestedProjects: suggestedProjects
        ? {
            ...
            materialConceptAttribution:
              suggestedProjects.materialConceptAttribution,
          }
        : null,
      canonicalHydration: {
        timingsMs: hydrationAudit.timingsMs,
        loadCanonicalMaterialConceptsMs,
        loadLearnerHomeContextMs:
          hydrationAudit.timingsMs.loadLearnerHomeContext ?? null,
      ...
      nPlusOne: {
        material_concept_chunk_bound: {
          status: chunkBoundStatus,
          materialConceptHydrationChunkSize:
            MATERIAL_CONCEPT_HYDRATION_CHUNK_SIZE,
          evidence: chunkBoundEvidence,
        },
        ...
        hydrationMaterialCandidateCount:
          hydrationAudit.candidateCounts.materials,
        hydrationProjectCandidateCount:
          hydrationAudit.candidateCounts.projects,
        hydrationRequiredComponentCount:
          hydrationAudit.candidateCounts.requiredComponents ?? null,
      },
    },
    callerMatrix,
    notes: [
  ...
  const reportPath = resolve(
    options.reportDir,
    `learner-home-performance-audit-${profile}-${Date.now()}.json`,
  );
...
export const runLearnerHomePerformanceAudit = async (
  options: AuditCliOptions,
) => {
  if (options.profile === 'canonical-isolated') {
    return withCanonicalIsolatedMlFlags(env, () =>
      runMeasuredAuditBody(options),
    );
  }
...
if (isMain) {
  ...
  runLearnerHomePerformanceAudit(options)
    .then(({ report, reportPath, status }) => {
      ...
      console.log(
        `profile=${options.profile} Cold median=${report.measurements.fullHomeCold.latencyMs.median}ms ` +
