...
const artifactPath = path.join(root, 'ml/recommendation/generated/portable-model/project-hybrid-runtime-v2.json');
...
const missingSamples: Array<{ missingFeatures: string[]; missingCount: number }> = [];
...
for (const candidate of candidates) {
  ...
  if (missing.length === 0) {
    ...
    missingSamples.push({ missingFeatures: missing.slice(0, 6), missingCount: missing.length });
...
console.log(JSON.stringify({
  ...
  projectReadinessStatus:
    nonFinite > 0 || duplicateRuntime > 0 || duplicateScored > 0 || candidates.length - artifactMappedCandidateCount > 0
      ? 'NOT_READY'
      : 'READY',
  ...
  outsideRuntimeArtifactCount: artifact.item_features.filter((feature) => !feature.name.startsWith('category:')
    && !feature.name.startsWith('concept:')
    && !feature.name.startsWith('difficulty:')
    && !feature.name.startsWith('component:')).length,
  ...
  hydrationFailureCount: duplicateRuntime + scoredKeys.filter((key) => !new Set(runtimeKeys).has(key)).length,
  ...
  unmappedProjects: candidates
    .map((candidate) => {
      ...
      return missing.length > 0
        ? { componentConceptKeys: candidate.componentConceptKeys, missingFeatures: missing }
        : null;
    })
    .filter(Boolean),
}));
