...
String incidentWorkflowLabel(String? workflowType) {
  switch (workflowType?.trim().toUpperCase()) {
    ...
    case 'ACCOUNTABILITY_AND_RECOVERY':
      return 'Accountability + recovery';
    default:
...
IncidentStrikeDisplay incidentStrikeDisplay(
  ...
  if (!hasTargetUser || strikeImpact?.trim().toUpperCase() == 'NONE') {
    return const IncidentStrikeDisplay(
      'No strike',
...
String incidentActionLabel(String action) {
  switch (action.trim().toUpperCase()) {
    case 'VERIFY':
      return 'Verify responsibility';
    case 'REJECT':
      return 'Reject report';
    case 'RESOLVE_WITHOUT_STRIKE':
      return 'Resolve without strike';
    case 'REQUEST_SUPPLIER_RESCHEDULE':
      return 'Ask supplier for new pickup window';
    case 'CANCEL_AND_RELEASE_HOLD':
      return 'Cancel and release hold';
    default:
      return action;
  }
}
...
String incidentActionSuccessLabel(String action) {
  switch (action.trim().toUpperCase()) {
    ...
    case 'RESOLVE_WITHOUT_STRIKE':
      return 'Incident resolved without a strike.';
    case 'REQUEST_SUPPLIER_RESCHEDULE':
      return 'Supplier asked to choose a new pickup window.';
    case 'CANCEL_AND_RELEASE_HOLD':
      return 'Reservation cancelled and hold released.';
    default:
