...
Future<void> showPickupQrDialog({
  ...
  // Keep the family provider alive while the dialog is open and retain
  // credential state for reopen while the detail page still watches it.
  ref.read(handoverCredentialControllerProvider(reservationId));

  return showDialog<void>(
    context: context,
    builder: (dialogContext) {
      return PickupQrDialog(
...
class PickupQrDialog extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PickupQrDialog> createState() => _PickupQrDialogState();
}

class _PickupQrDialogState extends ConsumerState<PickupQrDialog> {
  Timer? _expiryTimer;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      unawaited(
        ref
            .read(
              handoverCredentialControllerProvider(widget.reservationId).notifier,
            )
            .ensureIssued(),
      );
  ...
  void _scheduleExpiryCheck() {
    ...
    final credential = ref
        .read(handoverCredentialControllerProvider(widget.reservationId))
        .credential;
    ...
    // Single timer at expiry — avoids per-second rebuilds.
  ...
  Widget build(BuildContext context) {
    ...
    final state = ref.watch(
      handoverCredentialControllerProvider(widget.reservationId),
    );
    ...
    final showExpired =
        state.phase == HandoverCredentialPhase.error &&
        (state.error == 'EXPIRED' || (credential?.isExpiredAt() ?? false));

    return AppDialogShell(
      key: const Key('pickup-qr-dialog'),
      maxWidth: 420,
      title: Text(l10n.pickupQrDialogTitle),
      content: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Text(
            l10n.pickupQrInstructions,
          ...
          if (state.isLoading)
          ...
          else if (state.phase == HandoverCredentialPhase.error)
            _ErrorPanel(
            ...
            _ErrorPanel(
              message: l10n.pickupQrIssueFailed,
              onRetry: state.isLoading
                  ? null
                  : () => unawaited(controller.ensureIssued()),
            ),
  ...
  String _issueErrorMessage(Object? error, AppLocalizations l10n) {
    if (error == null) return l10n.pickupQrIssueFailed;
    if (error == 'EXPIRED') return l10n.pickupQrExpired;
    if (error is ApiException && error.code == 'PAYMENT_REQUIRED') {
      return l10n.pickupCodeLockedPayment;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Center(
          child: Semantics(
            label: l10n.pickupQrDialogTitle,
            image: true,
            excludeSemantics: true,
            child: Container(
              key: const Key('pickup-qr-image'),
        ...
        const SizedBox(height: AppSpacing.md),
        Text(
          l10n.pickupQrValidUntil(expiresLabel),
          textAlign: TextAlign.center,
          style: AppTextStyles.label(context).copyWith(
            color: palette.textSecondary,
        ...
        const SizedBox(height: AppSpacing.sm),
        Text(
          l10n.pickupQrManualFallbackNote,
          textAlign: TextAlign.center,
          style: AppTextStyles.label(context).copyWith(
    ...
    return Column(
      ...
      children: [
        ...
        if (onReissue != null) ...[
          const SizedBox(height: AppSpacing.md),
          AppPrimaryButton(
            key: const Key('pickup-qr-reissue'),
            label: l10n.pickupQrGenerateNew,
            onPressed: onReissue,
          ),
        ],
      ],
