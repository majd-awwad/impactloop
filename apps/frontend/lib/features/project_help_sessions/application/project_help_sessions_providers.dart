...
enum LearnerHelpSessionListFilter { all, active, scheduled, completed, closed }
...
final learnerHelpSessionsQueryProvider =
    NotifierProvider<
      LearnerHelpSessionsQueryNotifier,
      LearnerHelpSessionsQuery
    >(LearnerHelpSessionsQueryNotifier.new);

class LearnerHelpSessionsQueryNotifier
    extends Notifier<LearnerHelpSessionsQuery> {
  @override
...
final projectHelpSessionProjectOptionsProvider = FutureProvider.autoDispose
    .family<ProjectHelpSessionProjectOptionsResult, String?>((
      ref,
      query,
    ) async {
      await _ensureProjectHelpSessionAuthScope(ref);
...
final activeHelpSessionForBuildProvider = FutureProvider.autoDispose
    .family<ProjectHelpSession?, String>((ref, buildId) async {
      ...
      final result = await ref
          .read(projectHelpSessionsApiProvider)
          .fetchSessions(page: 1, limit: 5, buildId: buildId);
...
class ProjectHelpSessionActionController extends Notifier<bool> {
  ...
  Future<ProjectHelpSession?> createRequest({
    ...
    return _guard(
      () => ref
          .read(projectHelpSessionsApiProvider)
          .createRequest(buildId: buildId, payload: payload),
    );
  }
  ...
  Future<ProjectHelpSession?> acceptOption({
    ...
    return _guard(
      () => ref
          .read(projectHelpSessionsApiProvider)
          .acceptOption(sessionId: sessionId, timeOptionId: timeOptionId),
    );
  }
  ...
  Future<ProjectHelpSession?> proposeAlternative({
    ...
    return _guard(
      () => ref
          .read(projectHelpSessionsApiProvider)
          .proposeAlternative(sessionId: sessionId, startsAtUtc: startsAtUtc),
    );
  }
  ...
  Future<ProjectHelpSession?> declineSession({
    ...
    return _guard(
      () => ref
          .read(projectHelpSessionsApiProvider)
          .declineSession(sessionId: sessionId, reason: reason),
    );
  }
  ...
  Future<ProjectHelpSession?> cancelAuthorSession({
    ...
    return _guard(
      () => ref
          .read(projectHelpSessionsApiProvider)
          .cancelAuthorSession(sessionId: sessionId, reason: reason),
    );
  }
  ...
  Future<ProjectHelpSessionNotebookHandoff?> ensureNotebookNotes({
    ...
    return _guard(
      () => ref
          .read(projectHelpSessionsApiProvider)
          .ensureNotebookNotes(sessionId: sessionId, localeCode: localeCode),
    );
  }
  ...
  Future<ProjectHelpSessionSettings?> saveSettings({
    ...
    return _guard(
      () => ref
          .read(projectHelpSessionsApiProvider)
          .updateSettings(projectId: projectId, settings: settings),
    );
  }
}
...
class AuthorHelpSessionsQueryNotifier
    extends Notifier<AuthorHelpSessionsQuery> {
  @override
...
final authorHelpSessionsProvider =
    FutureProvider.autoDispose<ProjectHelpSessionListResult>((ref) async {
      ...
      final sorted = items.toList()
        ..sort((a, b) {
          final priority = _authorSessionPriority(
            a.status,
      ...
      return ProjectHelpSessionListResult(
        ...
        limit: result.limit,
        total:
            query.filter == AuthorHelpSessionListFilter.all ||
                query.filter == AuthorHelpSessionListFilter.scheduled
            ? result.total
            : sorted.length,
        totalPages:
...
final hasAuthoredProjectSubmissionsProvider = FutureProvider.autoDispose<bool>((
  ref,
) async {
...
final authorHelpSessionsEntryVisibleProvider = FutureProvider.autoDispose<bool>(
  (ref) async {
    await _ensureProjectHelpSessionAuthScope(ref);
