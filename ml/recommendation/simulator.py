...
def _op(namespace: str, *parts: object) -> str:
    return hashlib.sha256((namespace + ":" + ":".join(map(str, parts))).encode()).hexdigest()
...
def _persona_rows(seed: int, cfg: dict[str, Any], interests: list[str]) -> list[dict[str, Any]]:
    ...
    for index in range(1, cfg["persona_count"] + 1):
        ...
        rows.append({
            "sim_user_key": f"sim_user_{index:04d}", "origin": SYNTHETIC_ORIGIN,
            "primary_interest": primary, "secondary_interest": second if rng.random() < .58 else "",
            ...
            "cohort": cohort, "simulator_seed": seed,
        })
    return rows


def _episodes(personas: list[dict[str, Any]], interests: list[str], seed: int) -> list[dict[str, Any]]:
    rng = random.Random(seed + 901)
    rows = []
    for p in personas:
        count = 0 if p["cohort"] == "stable" else rng.choices([0, 1, 2], [15, 65, 20], k=1)[0]
        ...
        for n in range(count):
            ...
            rows.append({
                ...
                "sim_user_key": p["sim_user_key"], "origin": SYNTHETIC_ORIGIN,
                "intent_key": intent, "start_day": start, "end_day": min(30, start + duration),
                "transition": "GRADUAL" if p["cohort"] == "gradual_shift" else "ABRUPT",
                "simulator_seed": seed,
            })
    return rows


def simulate(catalog: dict[str, Any], cfg: dict[str, Any], seed: int) -> dict[str, list[dict[str, Any]]]:
    random.seed(seed)
    ...
    for p in personas:
        ...
        for session_n, day in enumerate(days):
            ts = start + timedelta(days=day - 1, hours=rng.randint(8, 19), minutes=rng.randint(0, 40))
            domain = "project" if rng.random() < (variant["project_tendency"] + .25 * p["project_driven_tendency"]) else "material"
            pool = projects if domain == "project" else materials
            key_field = f"{domain}_key"
            eligible = [r for r in pool if datetime.fromisoformat(r["publication_timestamp"].replace("Z", "+00:00")) <= ts]
            ...
            policy_ranked = sorted(eligible, key=lambda item: (
                item["category_key"] != current,
                item["category_key"] != p["primary_interest"],
                hashlib.sha256(f"{seed}:{p['sim_user_key']}:{day}:{item[key_field]}".encode()).hexdigest(),
            ))
            ...
            for rank, item in enumerate(selected):
                source = "RANDOM_EXPLORATION" if item in random_items else "POLICY_SELECTED"
                imp_ts = ts + timedelta(seconds=rank * 5)
                imp_key = _op("impression", session_key, rank, item[key_field])
                utility = (
                    1.2 * (item["category_key"] == current)
                    + variant["loyalty"] * (item["category_key"] == p["primary_interest"])
                    + .35 * rng.random() + float(np_rng.normal(0, .45))
                )
                impressions.append({
                    "impression_key": imp_key, "session_key": session_key,
                    "sim_user_key": p["sim_user_key"], "entity_key": item[key_field],
                    "domain": domain, "timestamp_utc": imp_ts.isoformat().replace("+00:00", "Z"),
                    "day": day, "split": split_for_day(day), "rank": rank + 1,
                    "source": source, "engaged": False, "origin": SYNTHETIC_ORIGIN,
                    "simulator_seed": seed,
                })
                ...
                def add(action_type: str, op_group: str, outcome: str = "") -> None:
                    ...
                    actions.append({
                        "action_key": _op("action", seed, imp_key, len(actions), action_type),
                        "operation_key": _op("operation", seed, imp_key, op_group),
                        "impression_key": imp_key, "session_key": session_key,
                        "sim_user_key": p["sim_user_key"], "entity_key": item[key_field],
                        "domain": domain, "action_type": action_type, "outcome": outcome,
                        "timestamp_utc": action_ts.isoformat().replace("+00:00", "Z"),
                        "day": day, "split": split_for_day(day), "origin": SYNTHETIC_ORIGIN,
                        "simulator_seed": seed,
                    })
                    made_action = True
                scale = variant["action_scale"]
                if domain == "material" and rng.random() < event_probs["view"] * bounded * scale:
                    cap_key = (p["sim_user_key"], item[key_field], day)
                    if view_counts[cap_key] < cfg["repeat_view_cap_per_item_day"]:
                        view_counts[cap_key] += 1; add("view", f"view-{view_counts[cap_key]}")
                like_p = event_probs["material_like"] if domain == "material" else event_probs["project_like"]
                if rng.random() < like_p * bounded * scale:
                    add("like", "like")
                    if rng.random() < cfg["reversal_probabilities"]["like"]: add("unlike", "like")
                if domain == "material" and rng.random() < event_probs["reservation"] * bounded * scale:
                ...
                if domain == "project":
                    for positive, reverse, prob in [
                        ...
                        if rng.random() < prob * bounded * scale:
                            ...
                            if rng.random() < cfg["reversal_probabilities"][positive]: add(reverse, positive)
    ...
    splits = [{
        "operation_key": r["operation_key"], "sim_user_key": r["sim_user_key"],
        "entity_key": r["entity_key"], "domain": r["domain"], "split": r["split"],
        "origin": SYNTHETIC_ORIGIN, "simulator_seed": seed,
    } for r in resolved]
    return {
        "personas": personas, "impressions": impressions, "actions": actions,
        ...
        "splits": splits, "hidden_state": episodes,
    }


def _resolve(actions: list[dict[str, Any]], cfg: dict[str, Any], anchor: datetime) -> list[dict[str, Any]]:
    groups: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for action in actions:
        groups[action["operation_key"]].append(action)
    weights = {"view": .4, "like": 1.2, "save": 1.1, "follow": 1.0,
               "reservation_created": 1.0, "build_started": 1.0}
    outcome_weights = {"COMPLETED": 2.5, "ACCEPTED": 1.7, "MEANINGFUL_PROGRESS": 1.8,
                       "CANCELLED": .3, "REJECTED": .15, "EXPIRED": .1, "ABANDONED": .2}
    rows = []
    for operation, chain in sorted(groups.items()):
        ...
        if types[-1] in {"unlike", "unsave", "unfollow"}:
            continue
        last = chain[-1]
        base = outcome_weights.get(last["outcome"], weights.get(types[0], .6))
        age = (anchor - datetime.fromisoformat(last["timestamp_utc"].replace("Z", "+00:00"))).total_seconds() / 86400
        decay = .5 ** (max(0, age) / cfg["time_decay_half_life_days"])
        rows.append({
            "operation_key": operation, "sim_user_key": last["sim_user_key"],
            "entity_key": last["entity_key"], "domain": last["domain"],
            "resolved_action": types[-1], "outcome": last["outcome"],
            "weight": round(min(3.0, max(.05, base * decay)), 6),
            "timestamp_utc": last["timestamp_utc"], "split": last["split"],
            "origin": SYNTHETIC_ORIGIN, "simulator_seed": last["simulator_seed"],
        })
    return rows


def logical_hash(dataset: dict[str, list[dict[str, Any]]]) -> str:
    normalized = {name: sorted(rows, key=lambda row: json.dumps(row, sort_keys=True))
