...
| `admin-no-show-reports`        | **Implemented (Phase 6–7)** | Admin reservation incident queue; verify/reject/resolve without strike; **no-driver operational actions** (`request-supplier-reschedule`, `cancel-release-hold` for SYSTEM `NO_DRIVER_AVAILABLE`); supplier `submit-no-driver-pickup-window`; enriched detail (messages, history, strikes); 3 verified strikes auto-suspend + token revocation |
