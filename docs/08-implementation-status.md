...
| Learner reservation status UX | **Partial** | Partial-quantity reserve/cancel, awaiting-confirmation actions, material `availableQuantity`, `/learner/reservations` with overdue warning, follow-up messages, delivery request/status — [reservations.md](features/reservations.md), [learner-reservation-flow](flows/learner-reservation-flow.md) |
