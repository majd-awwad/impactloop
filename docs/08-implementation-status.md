...
| `learning-projects`            | **Implemented** | Public read list + detail (`PUBLISHED` only) with project like/follower counts and viewer liked/saved/followed state; learner like/unlike/save/unsave/follow/unfollow; learner `POST /submit` → `PENDING_REVIEW`; admin moderation module                                                       |
...
| `learning_hub`       | **Partial**                  | `GET /api/learning-projects` + categories; `POST/DELETE /api/learning-projects/:id/like`; `POST/DELETE /api/learning-projects/:id/save`; `POST/DELETE /api/learning-projects/:id/follow`; `POST /api/learning-projects/submit`; `/api/admin/learning-projects/*` moderation | List/detail + Home spotlight API-backed; add-draft submits for review; learner project likes/saves/follows; admin moderation at `/admin/learning-projects`; AI disabled; ratings hidden                                  | [learning-hub.md](features/learning-hub.md)           |
...
| Learning hub API → Flutter                   | **Partial**         | Read path wired: `/learning`, `/learning/:id`, Home spotlight; add-draft submission, project likes/saves/follows, and admin moderation wired; server-side page navigation exposed; AI/ratings/saved-project listing/followed-project listing/checklist/material linking pending                                                                       |
...
| Learning projects                     | Yes       | Yes (PUBLISHED only + learner submit + admin moderation) | **Partial** — browse/detail/spotlight API-backed; add-draft submits for admin review; admin moderation UI exists |
| Reviews                               | Yes       | No                                                       | No                                                                                                               |
...
| 5 Learning hub + AI  | Projects + AI matching           | **Partial** — read API wired (Hub list/detail + Home spotlight); add-draft submission, admin moderation, learner project likes/saves/follows, and server-side Learning Hub page navigation wired; AI/ratings/saved-project listing/followed-project listing/checklist/material linking **not implemented** |
...
| Learning hub           | [features/learning-hub.md](features/learning-hub.md)                     | [flows/learning-hub-browse-flow.md](flows/learning-hub-browse-flow.md)                                                                                       | **Partial** — read path API-backed; add-draft submit, admin moderation, learner project likes/saves/follows, and server-side page navigation wired; AI/ratings/saved-project listing/followed-project listing/checklist/material linking pending |


**Not covered as implemented:** AI agent, moderator portal, saved/followed-project listing, build checklist, realtime driver tracking stream, scheduled background reservation expiry cron, and QR polish remain pending/deferred.
