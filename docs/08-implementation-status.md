...
| `admin-learning-projects`      | **Implemented** | Learning project moderation endpoints for ADMIN and MODERATOR tokens: list/detail/approve/request changes/reject/hide/restore/archive                                                                                                                     |
...
| `learning-projects`            | **Implemented** | Public read list + detail (`PUBLISHED` only); learner `POST /submit` -> `PENDING_REVIEW`; admin/moderator moderation module                                                                                                                                |
| `invitations`                  | **Implemented** | Admin email invitations (mock/SMTP), validate/accept API, unified `driver_profiles`                                                                                                                                                                       |
| `reservations`                 | **Partial**     | Learner create/cancel + learner confirmation for `AWAITING_LEARNER_CONFIRMATION` + my reservations read with scheduling fields, `activeDelivery`, post-acceptance `pickupLocationFull`, derived overdue follow-up fields, reservation-scoped messages; partial-quantity holds; learner delivery request route mounted |
...
| `admin`                        | **Partial**     | Dashboard + invitations + supplier verification + approvals + materials moderation + learning project moderation + people management + impact analytics + audit logs + reservations/deliveries monitoring (read-only); operational actions remain partial |
...
| `material_discovery` | **Implemented**              | `GET /api/materials`, `POST /api/reservations`                                                                                        | API discovery + nearest sort, approximate map pins, saved-location selector, reservation CTA                                                                                                             | —                                                     |
...
| Learning projects                     | Yes       | Yes (PUBLISHED only + learner submit + admin/moderator moderation) | **Partial** — browse/detail/spotlight API-backed; add-draft submits for review; admin moderation UI exists |
| Reviews                               | Yes       | No                                                       | No                                                                                                               |
...
**Not covered as implemented:** AI agent, moderator portal, saved projects, followed content, project likes, build checklist, moderator-owned project review UI, realtime driver tracking stream, reservation expiry, and reservation detail page remain pending.
