...
- `learningProjectsProvider`, `learningProjectProvider`, and `projectCategoriesProvider` load Learning Hub list/detail/category data from `/api/learning-projects` and `/api/categories?type=PROJECT`. Learning Hub list/detail DTOs include `likesCount`, `followersCount`, viewer-specific `isLiked`, viewer-specific `isSaved`, and viewer-specific `isFollowing`; the detail like/save/follow toggles use repository `likeProject` / `unlikeProject` / `saveProject` / `unsaveProject` / `followProject` / `unfollowProject` with optimistic widget-local state and invalidate `learningProjectProvider(id)` after mutation.
