...
- `learningProjectsProvider`, `savedLearningProjectsProvider`, `followedLearningProjectsProvider`, `learningProjectProvider`, `projectBuildProvider`, `projectCategoriesProvider`, and `materialCategoriesProvider` load Learning Hub list/detail/build/category data from `/api/learning-projects`, `/api/learning-projects/me/saved`, `/api/learning-projects/me/followed`, `/api/learning-projects/:id/builds/me`, `/api/categories?type=PROJECT`, and `/api/categories?type=MATERIAL`. Learning Hub list/detail DTOs include `likesCount`, `followersCount`, nullable `ratingSummary`, viewer-specific `isLiked`, viewer-specific `isSaved`, and viewer-specific `isFollowing`; detail additionally includes `recentReviews` and `viewerReview`. List cards, Home spotlight cards, and detail controls use repository `likeProject` / `unlikeProject` / `saveProject` / `unsaveProject` / `followProject` / `unfollowProject` with optimistic widget-local state and invalidate `learningProjectProvider(id)`, `learningProjectsProvider`, `savedLearningProjectsProvider`, and `followedLearningProjectsProvider` after successful engagement mutation. The detail reviews section uses `reviewProject` / `deleteProjectReview`, then invalidates `learningProjectProvider(id)` after success.
...
- `myLearningProjectSubmissionsProvider` and `myLearningProjectSubmissionProvider` load learner-owned submission cards/detail from `/api/learning-projects/mine` and `/api/learning-projects/mine/:id`. `LearningProjectSubmissionEditPage` uses repository `updateMyLearningProjectSubmission` and `resubmitMyLearningProjectSubmission`; successful save/resubmit invalidates the submission detail and submissions list providers. This server edit flow is separate from `learningProjectDraftStorageProvider`, so editing a `CHANGES_REQUESTED` submission does not overwrite the local create-draft form.
- `ProjectBuildActionsPanel` reads `projectBuildProvider(projectId)` only for authenticated learner sessions, starts builds with repository `startBuild`, and routes to `/learning/:id/build`. `LearningProjectBuildPage` uses `projectBuildProvider(projectId)` plus repository `updateBuildItem`, `fetchMaterialCandidates`, `linkMaterial`, and `unlinkMaterial` to persist manual checklist changes and linked-material state without resetting the full page. **Reserve this material** navigates to `/materials/:materialId?projectId&buildItemId&returnTo=/learning/:id/build`; `MaterialDetailsPage` passes optional `buildItemId` into `CreateReservationRequest`, invalidates `projectBuildProvider(projectId)` after success, and returns to `returnTo` when present. Component **Browse all materials** actions navigate to `/materials?q=<component>`.
