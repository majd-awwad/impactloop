...
- `learningProjectsProvider`, `savedLearningProjectsProvider`, `followedLearningProjectsProvider`, `learningProjectProvider`, `projectBuildProvider`, and `projectCategoriesProvider` load Learning Hub list/detail/build/category data from `/api/learning-projects`, `/api/learning-projects/me/saved`, `/api/learning-projects/me/followed`, `/api/learning-projects/:id/builds/me`, and `/api/categories?type=PROJECT`. Learning Hub list/detail DTOs include `likesCount`, `followersCount`, nullable `ratingSummary`, viewer-specific `isLiked`, viewer-specific `isSaved`, and viewer-specific `isFollowing`; detail additionally includes `recentReviews` and `viewerReview`. List cards, Home spotlight cards, and detail controls use repository `likeProject` / `unlikeProject` / `saveProject` / `unsaveProject` / `followProject` / `unfollowProject` with optimistic widget-local state and invalidate `learningProjectProvider(id)`, `learningProjectsProvider`, `savedLearningProjectsProvider`, and `followedLearningProjectsProvider` after successful engagement mutation. The detail reviews section uses `reviewProject` / `deleteProjectReview`, then invalidates `learningProjectProvider(id)` after success.
