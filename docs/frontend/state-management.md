...
- `learningProjectsProvider`, `savedLearningProjectsProvider`, `followedLearningProjectsProvider`, `learningProjectProvider`, and `projectCategoriesProvider` load Learning Hub list/detail/category data from `/api/learning-projects`, `/api/learning-projects/me/saved`, `/api/learning-projects/me/followed`, and `/api/categories?type=PROJECT`. Learning Hub list/detail DTOs include `likesCount`, `followersCount`, nullable `ratingSummary`, viewer-specific `isLiked`, viewer-specific `isSaved`, and viewer-specific `isFollowing`; detail additionally includes `recentReviews` and `viewerReview`. List cards, Home spotlight cards, and detail controls use repository `likeProject` / `unlikeProject` / `saveProject` / `unsaveProject` / `followProject` / `unfollowProject` with optimistic widget-local state and invalidate `learningProjectProvider(id)`, `learningProjectsProvider`, `savedLearningProjectsProvider`, and `followedLearningProjectsProvider` after successful engagement mutation. The detail reviews section uses `reviewProject` / `deleteProjectReview`, then invalidates `learningProjectProvider(id)` after success.
...
- `ProjectBuildActionsPanel` keeps build checklist status in widget-local state only. It does not use a provider or backend table yet. Component **Find materials** actions navigate to `/materials?q=<component>`; `MaterialsDiscoveryPage.initialQuery` seeds the material search query from that route parameter.
