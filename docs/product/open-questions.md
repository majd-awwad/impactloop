...
| Delivery cost calculation and who sets `delivery_cost`? | **Resolved for current code** — reserve-time pricing stores delivery fee/currency/zone metadata and grouped delivery can preserve one delivery fee; **current MVP is CASH-only**; dormant electronic checkout/PSP infrastructure retained for future integration | [delivery](features/delivery.md), [reservations](features/reservations.md), [payments](features/payments.md) |
