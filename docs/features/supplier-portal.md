...
| Learner material requests | **Implemented** | Privacy-safe feed at `/supplier/material-requests`; suggest owned materials or publish-from-request; candidate ranking marks category-only/low-evidence results as weak while model identifiers and specific request terms strengthen real matches; dashboard insight for unanswered count |
