...
**Home response cache:** Full learner-home responses are cached per authenticated user for 45 seconds. Successful material Like/Unlike, learner-interest updates, saved-location changes, project Save/Unsave/Like/Unlike/Follow/Unfollow, and learner Project Build start/status/link mutations remove only that learner’s full-home cache entry, so the next home request immediately recomputes behavior- and location-based recommendations. Successful reservation transitions that can change shared availability (create, decline/cancel/expiry, completion, no-show/recovery, and automatic escalation) clear all full-home response entries after their transaction commits; active-to-active scheduling and confirmation transitions retain the cache. The shared material-feature cache stores only static, derived content features (taxonomy keys, normalized terms, and tags); each request attaches fresh candidate status, availability, location, counts, and card payload before scoring. Reservation and Project Build lifecycle invalidation never clear that static feature cache.
