...
| Learning spotlight | **Implemented** | **API-backed** — `GET /api/learning-projects` via `learningProjectsProvider` (limit 2), including project like/save/follow controls, rating summaries, and viewer saved/followed state |
| Activity updates | **Partial** | Delivery/reservation entry links to `/learner/reservations`; a dedicated Home saved-project section remains Coming Soon |
| Coming later (impact + AI) | **Frontend-only** | Empty / Coming Soon placeholders — no learner-facing APIs |

**Not documented as implemented:** learner reservation cancel, dedicated Home saved-project section, AI agent, learner impact analytics.
...
3. Learning spotlight loads up to 2 published projects from `learningProjectsProvider` (`GET /api/learning-projects`, `page=1`, `limit=2`) and shows project like/save/follow controls, rating summaries, plus saved/followed state when present.
...
10. Disabled / Coming Soon cards show info snackbars for the dedicated Home saved-project section, impact, and AI helper.
...
No home-specific tables. Suggested material engagement reads from `materials.views_count` and `material_likes` via the materials API; learning spotlight engagement reads `project_likes`, `project_follows`, `project_user_reviews`, and viewer-specific `project_saves`/`project_follows` through the Learning Hub API.
...
- No dedicated Home saved-project section, followed suppliers/categories, materials-for-saved-projects, free-near-you, or continue-build sections yet. Learning Hub has learner Saved/Following tabs.
