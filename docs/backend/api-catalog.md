...
**Reservation / material-request lifecycle persistence:** Background workers started from `server.ts` persist due lifecycle transitions. GET/list paths may **derive** effective status (e.g. show a due `PENDING` as `EXPIRED`) without mutating rows. Expiry deadline for `PENDING` is the earlier of `createdAt + 48h` (`PENDING_SUPPLIER_RESPONSE_HOURS`) and the latest preferred pickup/delivery window end; rows without parseable windows use the 48-hour timeout only. Persisted expiry sets `EXPIRED`, releases the quantity hold, and writes status history. Configure intervals via `RESERVATION_LIFECYCLE_INTERVAL_MS` (default 5m), `RESERVATION_LIFECYCLE_REMINDER_INTERVAL_MS` (default 1m for due driver reminders), and `MATERIAL_REQUEST_LIFECYCLE_INTERVAL_MS` (default 5m). Workers use Postgres advisory locks for multi-instance safety.

**Missed-pickup cleanup:** Accepted `PICKUP` reservations with no delivery rows auto-expire to `EXPIRED` when `now > pickupWindowEnd + 72h` (`MISSED_PICKUP_AUTO_CLOSE_GRACE_HOURS`). `rejectionReason` is set to `PICKUP_WINDOW_MISSED`. Hold is released; no automatic strikes. Persistence is performed by the reservation lifecycle worker (and by write-path helpers); list DTOs may derive the same effective state on read.
