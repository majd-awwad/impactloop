...
Learner build checklist endpoints are manual and do not perform AI/material matching. `GET /api/learning-projects/:id/builds/me` returns the viewer's build checklist or `data: null` before start. `POST /api/learning-projects/:id/builds/start` is idempotent per learner/project and initializes one `project_build_items` row per required component. `PATCH /api/learning-projects/:id/builds/me/items/:itemId` body: `{ status: "MISSING" | "ALREADY_OWNED" | "AVAILABLE" | "RESERVED" | "ALTERNATIVE", learnerNote?: string | null }`. Build DTOs include `project`, `items[]`, and `progress` (`{ total, ready, percent }`); `ready` counts only `ALREADY_OWNED`, `AVAILABLE`, and `ALTERNATIVE` items as build-ready. `MISSING` and `RESERVED` are not counted as ready.
