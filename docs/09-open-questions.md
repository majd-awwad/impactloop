...
| Delivery cost calculation and who sets `delivery_cost`? | **Resolved for current code** — reserve-time pricing stores delivery fee/currency/zone metadata and grouped delivery can preserve one delivery fee; Mock payment checkout/settlement is implemented when `PAYMENT_PROVIDER=mock` (real PSP not implemented) | [delivery](features/delivery.md), [reservations](features/reservations.md), [payments](features/payments.md) |
